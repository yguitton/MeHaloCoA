#new version 14/07/2014 with check in FUN2 for multiple m/z hits
#change in M1 M2 calculation it was ppm this is confusing pepole ask for 50ppm error and in fact the soft used 60 ppmerr
cldetect<-
function(outdir="",xsAnnotate,plotps=TRUE, deprof=FALSE, Thresh=30, m1=1.0030, m2=1.997, mdiff=0.05, ppmerr=50, val="into", mdiff2=0.001, myfile="halo_list.csv",myfile2="halo_list_short.csv", m1need=FALSE){
...
#test if several M1 or M2[,"mz"]
if (length(M1[,"mz"])>1){
    testvect<-M1[,"mz"]
    M1<-M1[which(abs(testvect-target-massadduct1)==min(abs(testvect-target-massadduct1))),]
    
}
if (length(M2[,"mz"])>1){
    testvect<-M2[,"mz"]
    M2<-M2[which(abs(testvect-target-massadduct2)==min(abs(testvect-target-massadduct2))),]
}
 ...
 print(paste("Tests Result for target=",target, "and pcgroup=",good[j]))
 print(test2)
 print(test1)
            #liste des tests
...
#Plot IFC
    ...
    if (length(which(mat[,"Cl OK"]=="TRUE"))>=1){
        arrows(as.numeric(mat[which(mat[,"Cl OK"]=="TRUE"),"RT"])*60, x1=as.numeric(mat[which(mat[,"Cl OK"]=="TRUE"),"RT"])*60, y0=max(colSums(EIC.matrix$EIC))*0.05, y1=0,col = "blue", lty = "solid", lwd = 2, length = 0.04, angle = 45, code = 1)
    }
}else
    {
        EIC.matrix<-getAllPeakEICs(an,index=rep(1,nrow(an@groupInfo)))
        ...
        plot(x=unlist(EIC.matrix$scantime),y=colSums(EIC.matrix$EIC), type="l", col="grey", main=paste("IFC for \n",basename(an@xcmsSet@filepaths), "\n No Cl or Br"), xlab="seconds", ylab="Intensity") #IFC 
    }
...
if (length(good[which(ls_good2>=1)])>=1){
    ...
    cat(paste("Results files are in", outdir))
    if (plotps==TRUE){
